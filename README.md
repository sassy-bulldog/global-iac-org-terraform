...
> In this guide, we will be using GitHub to host our repositories. You can find more information about other supported VCS providers in [the list of supported VCS providers](https://docs.spacelift.io/integrations/source-control).
...
Since this is meant to be a quick and easy-to-follow tutorial, we won't go into the details, but you can read more about them in [the stack concept documentation](https://docs.spacelift.io/concepts/stack).
...
Now, where did _that_ come from? In fact, we declared it using Terraform just in [the stack.tf file](./stack.tf). The same file defines many things related to the environment, so let's click on the name of the new stack to be taken to its screen.
